...
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE; -- в первом терминале
SET TRANSACTION ISOLATION LEVEL SERIALIZABLE; -- во втором терминале

SELECT * FROM pizzeria WHERE name = 'Pizza Hut'; -- в первом терминале
UPDATE pizzeria SET rating = 3.0 WHERE name = 'Pizza Hut'; -- во втором терминале
